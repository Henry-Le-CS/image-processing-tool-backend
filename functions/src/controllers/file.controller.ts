...
export const retrieveFiles = async (
    ...
    try {
        const fileResponseData = await retrieveFilesInDriveFolder(folderId, pageSize, nextPageToken);

        const files = fileResponseData?.data?.files;
        if (!(files && files.length > 0)) {
            return res.status(404).send(`Cannot find any files in the folder with Id: ${folderId}`)
        }

        const filesData = files.reduce((accumulator, currentFile) => {
        ...
        const newNextPageToken = filesData && filesData.length > 0 ? fileResponseData?.data?.nextPageToken : "";
        return res.status(200).send({ data: filesData, nextPageToken: newNextPageToken })
    }
    catch (err) {
...
export const renameFiles = async (req: Request<{}, {}, {
    ...
    const unsucessfulPromises = promiseResult.filter((promise) => promise.status === "rejected");
